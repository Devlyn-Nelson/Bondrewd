...
use super::common::{BitMath, EnumInfo, FieldInfo, StructInfo};
...
impl FieldInfo {
    fn get_quotes(&self, struct_info: &StructInfo) -> Result<FieldQuotes, syn::Error> {
        let flip = struct_info.get_flip();
        let mut math = BitMath::from_field(self)?;
        let operator = if let Some(flip) = flip {
            math.starting_inject_byte = flip - math.starting_inject_byte;
            quote! {-}
        } else {
            quote! {+}
        };
        ...
        // TODO from here we need to split the logic into:
        // - get_quotes_le
        // - get_quotes_be
        // - get_quotes_ne
        todo!("finish FieldInfo::get_quotes");
    }
}
