...
#[test]
fn cycle_header() -> anyhow::Result<()> {
    let mut header = AosFrameHeader {
    ...
    while header.virtual_channel_frame_count != end {
        ...
        AosFrameHeader::write_virtual_channel_frame_count(&mut bytes, header.virtual_channel_frame_count);
        assert_eq!(header.virtual_channel_frame_count, AosFrameHeader::read_virtual_channel_frame_count(&bytes))
    }
